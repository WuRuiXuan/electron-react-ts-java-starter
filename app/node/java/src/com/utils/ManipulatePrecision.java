...
public class ManipulatePrecision {
	...
	/**
	 * Begin 2015-07-24 tongs 精确度为角
	 */
	public static String doubleToString2(double value) {
		return doubleToString(value, 1, 1, false, 0);
	}

	/**
	 * End 2015-07-24 tongs 精确度为角
	 */
	...
	public static String[][] registerFunction = { { "1", "金卡工程" }, { "2", "双屏广告" }, { "4", "" }, { "8", "" },
			{ "16", "" } };
	...
	// {
	// if (PathFile.fileExist(App.configPath+"/ModelFunc.ini"))
	// {
	// BufferedReader br = null;
	// try
	// {
	// String modelfunc = null;
	// br = PathFile.readFile(App.configPath+"/ModelFunc.ini");
	// if (br != null)
	// {
	// String line = null;
	...
	// if (modelfunc != null)
	// {
	// String key = getRegisterCodeKey(AppConfig.CDKey);
	// String strseq = getRegisterCodeSeqno(modelfunc);
	// String code1 = ManipulatePrecision.getRegisterCode("","",strseq, key);
	// String code2 = ManipulatePrecision.getRegisterCode(AndroidPos
	// .sysPara!=null?AndroidPos.sysPara.mktname:"",
	// AndroidPos
	// .sysPara!=null?AndroidPos.sysPara.mktcode:"",
	// strseq, key);
	// if (modelfunc.startsWith(code1) || modelfunc.startsWith(code2))
	// {
	// return modelfunc;
	// }
	// }
	...
	public static String getRegisterCode(String cust, String mkt, String seq, String keys) {
		...
		if (len > 0) {
			...
			for (i = 1; i <= len; i++) {
				...
				// 第一步算法
				Num1 = ((Num1 + (user.charAt(i - 1) * i * i) * (i * user.charAt(i - 1) + 1) + k * keylen * j)) % 100000;

				// 第二步算法
				Num2 = (Num2 * i + (user.charAt(i - 1) * i) + k * keylen * j) % 100000;

				// 第三步算法
				Num3 = (Num2 + Num1 + (user.charAt(i - 1) * i + 1) + k * keylen * j) % 100000;

				// 第四步算法
				Num4 = (Num3 + Num2 + Num1 + (user.charAt(i - 1) * i) + k * keylen * j) % 100000;
			}
	...
	public static String readApplicationModuleType() {
		if (PathFile.fileExist(App.configPath + "moduletype.ini")) {
			BufferedReader br = null;
			try {
				br = PathFile.readFile(App.configPath + "moduletype.ini");
				if (br != null) {
					String line = null;
					while ((line = br.readLine()) != null) {
						if (line != null && line.trim().length() >= 4) {
							return line.trim().substring(0, 4);
						}
					}
					br.close();
					br = null;
	...
	/*
	 * //浮点转化为字符串 public static String getFloatConverStr(double f, int len, int dec,
	 * int flag) { String str = " "; String str1 = ""; int flen = 0;
	 * 
	 ...
	 * 
	 * return str1.substring(0); } else { flen = String.valueOf(f).length(); str1 =
	 * String.valueOf(f);
	 * 
	 ...
	 * 
	 * //获得文件大小 public static long getFileLength(String filename) { File file = new
	 * File(filename);
	 * 
			...
			while (lenIntFen != 0) {
				...
				if ((i == 1) && (strFen.charAt(lenIntFen - 1) == '0')) {
					...
					strBig = "零" + strBig;
						...
						&& (strNow.charAt(1) == '亿') && (strNow.charAt(3) != '仟')) {
					strBig = strNum + strDW + strBig.substring(1, strBig.length());
				} else if ((i < 15) && (i > 11) && (strFen.charAt(lenIntFen - 1) != '0') && (strNow.charAt(0) == '零')
						&& (strNow.charAt(1) == '亿') && (strNow.charAt(3) == '仟')) {
