...
/**
 * Created by zhaoshanshan on 2017/11/26.
 */
...
public class ExpressionDeal {
	...
	public static String SpiltExpression(String strExpression) {
		...
		while (strExpression.indexOf("(") != -1) {
			...
			// strExpression=strExpression.replace("("+strExp+")",String.valueOf(CalculateExpress
			// (strExp)));
			strExpression = replace(strExpression, "(" + strExp + ")", String.valueOf(CalculateExpress(strExp)));
		}

		if (strExpression.indexOf("+") != -1 || strExpression.indexOf("-") != -1 || strExpression.indexOf("*") != -1
				|| strExpression.indexOf("/") != -1 || strExpression.indexOf("\\") != -1
				|| strExpression.indexOf("%") != -1 || strExpression.toUpperCase().indexOf("IF?") != -1) {
	...
	private static double CalculateExpress(String strExpression) {
		...
		// 只有IF?，则添加操作符进行函数运算
		if (strExpression.trim().toUpperCase().startsWith("IF?"))
		...
		while (strExpression.indexOf("+") != -1 || strExpression.indexOf("-", minus) != -1
			...
			if (strExpression.indexOf("*") != -1) {
				...
				// strExpression=strExpression.replace(strOne+"*"+strTwo,String.valueOf
				// (ReplaceValue));
				...
				ReplaceValue = ManipulatePrecision.integerDiv(parseDouble(GetExpType(strOne)),
						parseDouble(GetExpType(strTwo)));
		...
		case Iif: {
			...
			if (s.length >= 3) {
				...
				if (exp.indexOf("LS") >= 0) {
					...
					if (v.length >= 2 && parseDouble(v[0]) < parseDouble(v[1]))
						ok = true;
					else
						ok = false;
				} else if (exp.indexOf("GR") >= 0) {
					String[] v = exp.split("GR");
					if (v.length >= 2 && parseDouble(v[0]) > parseDouble(v[1]))
						ok = true;
					else
						ok = false;
				} else if (exp.indexOf("EQ") >= 0) {
					String[] v = exp.split("EQ");
					if (v.length >= 2 && parseDouble(v[0]) == parseDouble(v[1]))
						ok = true;
					else
						ok = false;
				} else if (exp.indexOf("LE") >= 0) {
					String[] v = exp.split("LE");
					if (v.length >= 2 && parseDouble(v[0]) <= parseDouble(v[1]))
						ok = true;
					else
						ok = false;
				} else if (exp.indexOf("GE") >= 0) {
					String[] v = exp.split("GE");
					if (v.length >= 2 && parseDouble(v[0]) >= parseDouble(v[1]))
						ok = true;
					else
						ok = false;
				} else if (exp.indexOf("NE") >= 0) {
				...
				// 条件判断为真
				if (ok)
					retValue = parseDouble(s[1]);
				else
					retValue = parseDouble(s[2]);
			}
			break;
		}
		}
	...
	private static int GetPrivorPos(String strExpression) {
		...
		// 负号,IF?
		if (strExpression.trim().charAt(0) == '-')
			return -1;
		if (strExpression.trim().toUpperCase().startsWith("IF?"))
			return -1;
	...
	private static String GetExpType(String strExpression) {
		...
		if (strExpression.indexOf("SIN") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("N") + 1, strExpression.length()), Sin));
		}

		if (strExpression.indexOf("COS") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("S") + 1, strExpression.length()), Cos));
		}

		if (strExpression.indexOf("TAN") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("N") + 1, strExpression.length()), Tan));
		}

		if (strExpression.indexOf("ATAN") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("N") + 1, strExpression.length()), ATan));
		}

		if (strExpression.indexOf("SQRT") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("T") + 1, strExpression.length()), Sqrt));
		}

		if (strExpression.indexOf("POW") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("W") + 1, strExpression.length()), Pow));
		}

		if (strExpression.indexOf("IF?") != -1) {
			return String.valueOf(CalculateExExpress(
					strExpression.substring(strExpression.indexOf("?") + 1, strExpression.length()), Iif));
		}
	...
	public static String StringExpress(String strExpression) {
		...
		if (strExpression.toUpperCase().startsWith("SUB"))
