...
public class Convert {
	...
	public static String appendStringSize(String orgLine, String addLine, int startIndex, int length, int orgSize,
		...
		int addLength = countLength(addLine);
		...
		if (addLine != null) {
			// 取需要添加字符串的最小值为指定添加长度
			if (addLength > length) {
				overLine = newSubString(addLine, length, addLength);
				...
				case 0:
					...
					break;
				case 1:
					sub = (length - addLength);
					break;
				case 2:
					sub = (length - addLength) / 2;
					break;
				}

				for (int i = addLength; i < length; i++) {
					if (sub > 0) {
						addLine = " " + addLine;
						sub--;
					} else {
			...
			if (overLine.length() > 0) {
				int appendNum = Integer.valueOf(String.valueOf((length - getWordCount(overLine)) / 2));
